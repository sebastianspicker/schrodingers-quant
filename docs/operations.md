...
Runbooks: [Debian 13 VPS](../ops/README.md) (host setup, systemd,
control), [soak checklist](../ops/soak-checklist.md),
[live pilot](live-pilot.md), [Jev](jev.md).
...
| `make test` | Build the `test` image and run pytest inside it (repo mounted read-only) |
| `make ci` | All of the above |
| `make research ARGS="…"` | `research/run.sh` subcommands |
| `make preflight ARGS="--pair BTC/EUR --stake 8 --stoploss -0.20"` | Read-only Kraken feasibility check |
| `make reconcile` | Read-only DB vs Kraken comparison; needs secrets |
| `make backup` | `ops/backup.sh`: every `user_data/runtime/*.sqlite` plus Jev records |
| `make retention` | `ops/retention.sh`: dangling-image pruning (dry-run unless `ARGS=--apply`) |
...
(profile `tools`); the Jev worker starts only with `docker compose --profile jev up -d jev-worker`.

## Secrets and local overlays

Copy `config/examples/secrets.example.json` to `config/local/secrets.json`
and `compose.override.example.yaml` to `compose.override.yaml` (both ignored).
Compose replaces the `command` list, so the override restates it with the extra
`--config`. Replace every placeholder, including the API `jwt_secret_key`. Never
commit credentials, databases, account exports or market data, and never paste
secrets into terminals, tickets or chat.
